...
class SqlRemoveHelperTest {
    ...
    void testRemoveSameFieldFromSelect() {
        String sql =
                "select 歌曲名,歌手名,粉丝数,粉丝数,sum(粉丝数),sum(粉丝数),avg(播放量),avg(播放量)"
        ...
        sql =
                "SELECT 结算播放量 FROM 艺人 WHERE (歌手名 IN ('林俊杰', '陈奕迅')) AND (数据日期 >= '2024-04-04' AND 数据日期 <= '2024-04-04')";
        List<FieldExpression> fieldExpressionList = SqlSelectHelper.getWhereExpressions(sql);
        fieldExpressionList.stream()
                .forEach(
                        fieldExpression -> {
    ...
    void testRemoveWhereHavingCondition() {
        String sql =
                "select 歌曲名 from 歌曲库 where sum(粉丝数) > 20000 and  2>1 and "
                        + "sum(播放量) > 20000 and 1=1  HAVING sum(播放量) > 20000 and 3>1";
        ...
        Assert.assertEquals(
                ...
                sql);
        sql =
                "SELECT 歌曲名,sum(播放量) FROM 歌曲库 WHERE (1 = 1 AND 1 = 1 AND 2 > 1 )"
        ...
        sql =
                "SELECT 品牌名称,法人 FROM 互联网企业 WHERE (2 > 1 AND 1 = 1) AND 数据日期 = '2023-10-31'"
    ...
    void testRemoveGroupBy() {
        String sql =
                "select 数据日期 from 歌曲库 where 歌曲名 = '邓紫棋' and 数据日期 = '2023-08-09' and "
