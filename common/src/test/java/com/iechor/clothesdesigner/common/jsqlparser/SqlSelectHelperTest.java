...
class SqlSelectHelperTest {
    ...
    void testGetWhereFilterExpression() {
        ...
        List<FieldExpression> fieldExpression =
                SqlSelectHelper.getFilterExpression(
        ...
        fieldExpression =
                SqlSelectHelper.getFilterExpression(
                        "SELECT sum(销量) / (SELECT sum(销量) FROM 营销月模型 "
    ...
    void testGetAllFields() {
        List<String> allFields =
                SqlSelectHelper.getAllSelectFields(
                        "SELECT department, user_id, field_a FROM s2 WHERE sys_imp_date = '2023-08-08'"
                                + " AND user_id = 'alice' AND publish_date = '11' ORDER BY pv DESC LIMIT 1");
        ...
        allFields =
                SqlSelectHelper.getAllSelectFields(
                        "SELECT sum(销量) / (SELECT sum(销量) FROM 营销 "
    ...
    void testGetWhereFields() {
        ...
        sql =
                "select 部门,用户 from 超音数 where 数据日期 = '2023-08-08'"
    ...
    void testGetPureSelectFields() {
        String sql =
                "select TIMESTAMPDIFF(MONTH, 发布日期, '2018-06-01')  from `超音数` "
        ...
        sql =
                "select 发布日期,数据日期,TIMESTAMPDIFF(MONTH, 发布日期, '2018-06-01')  from `超音数` where "
