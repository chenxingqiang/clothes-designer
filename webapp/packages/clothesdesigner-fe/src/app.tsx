...
import { Copilot } from 'clothesdesigner-chat-sdk';
...
const getRunningEnv = async () => {
  try {
    const response = await fetch(`${publicPath}clothesdesigner.config.json`);
...
export function onRouteChange() {
  const title = window.document.title.split('-clothesdesigner')[0];
  if (!title.includes('clothesdesigner')) {
    window.document.title = `${title}-clothesdesigner`;
  } else {
    window.document.title = 'clothesdesigner';
  }
}
