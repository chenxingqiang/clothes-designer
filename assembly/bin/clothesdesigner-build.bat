...
set "sbinDir=%~dp0"
call %sbinDir%/clothesdesigner-common.bat %*
...
if "%service%"=="webapp" (
   ...
   move /y clothesdesigner-webapp webapp
...
:buildWebapp
   ...
   copy /y clothesdesigner-webapp.tar.gz %buildDir%\
...
:packageRelease
   set "model_name=%service%"
   set "release_dir=clothesdesigner-%model_name%-%MVN_VERSION%"
   ...
   tar xvf clothesdesigner-webapp.tar.gz
   move /y clothesdesigner-webapp webapp
   echo {"env": ""} > webapp\clothesdesigner.config.json
   ...
   del clothesdesigner-webapp.tar.gz
